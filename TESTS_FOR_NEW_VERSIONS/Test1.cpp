...
void cadastrar_matricula(Matricula matriculas[], int *num_matriculas, int idAluno, Curso cursos[], int num_cursos) {
    ...
    for (int i = 0; i < num_cursos; i++) {
        if (cursos[i].id == idCurso) {
            ...
            // Criar vencimentos
            criar_vencimentos(vencimentos, num_vencimentos, matriculas[*num_matriculas], cursos[i]);
    ...
    for (int i = 0; i < num_alunos; i++) {
        if (alunos[i].id == id && strcmp(alunos[i].senha, senha) == 0) {
            ...
            do {
                printf("\n======= Menu do Aluno =======\n");
                printf("1. Ver cursos disponíveis\n");
                printf("2. Matricular em um curso\n");
                printf("3. Ver minhas matrículas\n");
                printf("4. Ver meus vencimentos\n");
                ...
                switch (opcao_aluno) {
                    case 1:
                        ...
                        break;
                    case 2:
                        cadastrar_matricula(matriculas, num_matriculas, id, cursos, num_cursos);
                        break;
                    case 3:
                    ...
                    case 5: {
                        int idVencimento;
                        printf("Digite o ID do vencimento a ser pago: ");
                        scanf("%d", &idVencimento);
                        getchar(); // Limpa o buffer de entrada

                        for (int i = 0; i < *num_vencimentos; i++) {
                            if (vencimentos[i].idVencimento == idVencimento && vencimentos[i].pago == 0) {
                                vencimentos[i].pago = 1;
                ...
                case 3: {
                    ...
                    do {
                        ...
                        switch (opcao_matricula) {
                            case 1:
                                cadastrar_matricula(matriculas, num_matriculas, 0, cursos, num_cursos);
                                break;
                            case 2: {
                                int idMatricula;
                                printf("Digite o ID da matrícula a ser editada: ");
