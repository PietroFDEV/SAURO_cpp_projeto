    ...
    for (int i = 0; i < num_vencimentos; i++) {
        if (vencimentos[i].idVencimento == id) {
            ...
            if (strcmp(novo_valor, "\n") != 0) {
                vencimentos[i].valorParcela = atof(novo_valor);
            }
            ...
            if (strcmp(nova_data, "\n") != 0) {
                ...
                int year, month, day;
                if (sscanf(nova_data, "%d-%d-%d", &year, &month, &day) == 3) {
                    ...
                    time_t t = mktime(&tm);
                    vencimentos[i].dataVencimento = t;
                }
            }
...
void salvar_vencimentos(Vencimento vencimentos[], int num_vencimentos) {
    ...
    for (int i = 0; i < num_vencimentos; i++) {
        fprintf(file, "%d %d %.2f %d %s\n", vencimentos[i].idVencimento, vencimentos[i].idMatricula, vencimentos[i].valorParcela, vencimentos[i].pago, vencimentos[i].dataVencimento);
    }
